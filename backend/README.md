...
подгрузка моделью через тулзу `get_pattern({"name": "..."})` в момент нужды
(перед первым `execute_select`); в промпте агентов только указатель.
Новый паттерн — новый файл, без рестарта.
