...
- Агенты: `backend/agents/<name>/AGENT.md` (assistant/analyst/tz-helper, формат — `backend/agents/AGENT.md`); промпт агента ЗАМЕНЯЕТ базовый, `tools` режут реестр (MCP один, фильтром), `skills` — разрешённые скилы, `model` — опциональный оверрайд. Выбор: явный `agent` из дропдауна 1С (дефолт `assistant`) → залипший в `sessions.agent_name`. RAG изолирован (`documents.agent_name`, NULL — общий). `GET /agents` — список для формы. Новый агент — файл без кода и рестарта.
- Скилы: `backend/skills/<name>/SKILL.md` (name/description/tools + промпт, формат — `backend/skills/SKILL.md`); бэкенд (`app/skills/loader.py`) фильтрует `ToolRegistry` и добавляет промпт к промпту агента. Выбор: явный `skill` из дропдауна 1С → залипший в `sessions.skill_name` → авто-матчинг по `description` (эмбеддинги, top-1, только среди скилов агента). `GET /skills?agent=` — список для формы. Новый сценарий — файл без кода и рестарта.
